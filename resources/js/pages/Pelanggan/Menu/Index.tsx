...
interface Props extends PageProps {
    ...
    eceran: Record<EceranSubType, MenuItem[]>;
}
...
const sectionMeta: Record<
    SectionConfig['id'],
    Pick<SectionConfig, 'title' | 'description' | 'eyebrow' | 'badge'>
> = {
    timbang_hidup: {
        title: 'Timbang hidup',
        description:
            'Pilih potongan berdasarkan berat, kondisi, dan tier harga.',
        eyebrow: 'Menu utama',
        badge: 'Berat',
    },
    paket_nasi_box: {
    ...
    babi_adat: {
        title: 'Babi Adat',
        description: 'Paket lengkap untuk kebutuhan adat, harga sudah all-in.',
        eyebrow: 'Eceran',
        badge: 'Adat',
    },
    paket_pass: {
        title: 'Paket pass',
        description: 'Paket siaga untuk kebutuhan cepat tanpa banyak langkah.',
        eyebrow: 'Eceran',
        badge: 'Paket',
    },
};
...
function collapseTimbangItems(items: MenuItem[]): MenuItem[] {
    ...
    for (const item of items) {
        ...
        // pick the lowest min_price
        if (
            item.min_price !== null &&
            (existing.min_price === null || item.min_price < existing.min_price)
        ) {
...
function Index({ timbang_hidup, eceran }: Props) {
    ...
    const [initialVariantId, setInitialVariantId] = useState<string | null>(
        null,
    );
    ...
    const allSections = useMemo<SectionConfig[]>(() => {
        const sections = [
            {
                id: 'timbang_hidup',
                items: collapseTimbangItems(timbang_hidup),
            },
            {
                id: 'paket_pass',
                items: eceran.paket_pass,
            },
            {
                id: 'paket_nasi_box',
                items: eceran.paket_nasi_box,
            },
            {
                id: 'babi_adat',
                items: eceran.babi_adat,
            },
        ];
    ...
    function handleSelectItem(
        ...
        setInitialVariantId(opts?.variantId ?? null);
                                ...
                                <div className="flex items-center gap-2 text-xs text-slate-500">
                                    ...
                                    {query.trim() !== '' && (
                                        <button
                                            type="button"
                                            onClick={() => setQuery('')}
                                            className="rounded-full border border-black/5 px-3 py-2 font-medium text-primary transition hover:border-primary/20 hover:bg-primary/5"
                                        >
                                            Reset
                                        </button>
                                    )}
                        ...
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                                ...
                                <p className="mt-1 text-sm font-medium text-text">
                                    Lompat ke section yang paling relevan.
                                </p>
                            </div>

                            <button
                                type="button"
                                onClick={() => router.visit('/user/keranjang')}
                                className="inline-flex items-center justify-center gap-2 rounded-full border border-black/5 bg-[#fbfaf6] px-4 py-2 text-sm font-semibold text-text transition hover:border-primary/20 hover:text-primary"
                            >
                                Lihat keranjang
