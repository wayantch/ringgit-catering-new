...
it('returns only diproses orders grouped by booking date and sorted by time', function (): void {
    ...
    $response->assertInertia(
        fn ($page) => $page
            ...
            ->where('printData.groups.0.orders.0.customer_name', 'Cetak Satu')
            ->where('printData.groups.0.orders.0.items.0.detail_label', 'Babi Test Print '.$uniqueSuffix.' — Adat Batak — Kepala, Ekor')
            ->where('printData.groups.0.orders.0.payment_method', 'dp')
            ->where('printData.groups.0.orders.0.payment_date', '02/01/2037')
            ->where('printData.groups.0.orders.0.jam', '07:15')
            ->where('printData.groups.0.orders.1.customer_name', 'Cetak Dua')
            ->where('printData.groups.0.orders.1.items.0.detail_label', 'Ayam Panggang '.$uniqueSuffix.' — Panggang')
            ->where('printData.groups.0.orders.1.payment_method', 'pembayaran penuh')
            ->where('printData.groups.0.orders.1.payment_date', '03/01/2037')
            ->where('printData.groups.0.orders.1.jam', '09:30')
